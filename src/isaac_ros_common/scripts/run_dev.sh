...
# Re-use existing container (running or stopped).
if [ "$(docker ps -a --quiet --filter name=$CONTAINER_NAME)" ]; then
    if [ "$(docker ps --quiet --filter status=running --filter name=$CONTAINER_NAME)" ]; then
        ...
        print_info "Starting existing stopped container: $CONTAINER_NAME"
...
# Mount RealSense camera devices
# RealSense uses /dev/video* for video streams and /dev/ttyACM* for control
# Mount video devices (primary - for actual image streams)
DOCKER_ARGS+=("-v /dev/video:/dev/video")

# Mount USB serial devices (for RealSense control + motor communication)
# Mount all ttyACM devices (RealSense and other USB serial devices)
for tty_device in /dev/ttyACM*; do
    if [[ -e "$tty_device" ]]; then
        DOCKER_ARGS+=("--device=$tty_device")
    fi
done

# Also mount common USB serial device patterns
# Some devices appear as ttyUSB* instead of ttyACM*
...
# Mount entrypoint additions from workspace if they exist
if [ -d "$ROOT/../docker/scripts/entrypoint_additions" ]; then
    DOCKER_ARGS+=("-v $ROOT/../docker/scripts/entrypoint_additions:/usr/local/bin/scripts/entrypoint_additions:ro")
fi
